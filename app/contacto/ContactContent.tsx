...
/**
 * Jugador por el que se pregunta, cuando se llega desde su ficha con
 * ?jugador=<slug>. Lo resuelve la página servidor: aquí solo se muestra y se
 * adjunta al envío, para que la agencia sepa de quién se trata sin depender de
 * que quien escribe lo teclee.
 */
export type JugadorConsultado = { id: string; nombre: string; href: string };

export default function ContactContent({
  jugador,
}: {
  jugador?: JugadorConsultado | null;
}) {
  const [formData, setFormData] = useState({
    nombre: "",
    email: "",
    telefono: "",
    asunto: jugador ? "jugador" : "",
    mensaje: jugador
      ? `Hola, quiero solicitar información sobre ${jugador.nombre}.`
      : "",
    oficina: "rd",
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2">
              <div className="card-dark rounded-2xl p-8">
                ...
                {jugador && (
                  <div className="mb-8 flex flex-wrap items-center gap-x-2 gap-y-1 p-4 rounded-lg border border-gold/40 bg-gold/10">
                    <span className="text-xs font-bold uppercase tracking-widest text-gold-dark">
                      Jugador
                    </span>
                    <Link
                      href={jugador.href}
                      className="font-bold text-gold hover:text-gold-light underline underline-offset-4"
                    >
                      {jugador.nombre}
                    </Link>
                    <span className="text-xs text-body/60">
                      · Tu consulta se enviará asociada a esta ficha.
                    </span>
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <option value="jugador">Información sobre un jugador</option>
